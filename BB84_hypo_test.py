...
class RandomBasisQBERTest:
    ...
    def binomial_likelihood(self, k, n, p):
        """
        Calculate binomial likelihood P(k errors | n trials, error probability p)
        Using log-likelihood to avoid numerical overflow
        """
        if p <= 0 or p >= 1:
            return 0 if k == 0 or k == n else 1e-100
        
        # Log-likelihood for numerical stability
        log_binom_coeff = math.lgamma(n + 1) - math.lgamma(k + 1) - math.lgamma(n - k + 1)
        log_likelihood = log_binom_coeff + k * math.log(p) + (n - k) * math.log(1 - p)
        
        return math.exp(log_likelihood)
    
    def eve_detection_ml(self, observed_errors, n_bits):
        """
        Maximum Likelihood Eve detection
        
        Args:
            observed_errors: Number of observed errors
            n_bits: Total number of sifted bits
            
        Returns:
            dict with detection results
        """
        # Calculate likelihoods for both hypotheses
        p_no_eve = self.theoretical_p1_no_eve
        p_with_eve = self.theoretical_p1_with_eve
        
        # Likelihood under H0 (no Eve)
        likelihood_no_eve = self.binomial_likelihood(observed_errors, n_bits, p_no_eve)
        
        # Likelihood under H1 (Eve present)
        likelihood_with_eve = self.binomial_likelihood(observed_errors, n_bits, p_with_eve)
        
        # Likelihood ratio
        if likelihood_no_eve > 0:
            likelihood_ratio = likelihood_with_eve / likelihood_no_eve
        else:
            likelihood_ratio = float('inf') if likelihood_with_eve > 0 else 1.0
        
        # Decision based on likelihood ratio
        eve_detected = likelihood_ratio > 1.0
        
        # Calculate confidence (how much more likely is the chosen hypothesis)
        confidence = max(likelihood_ratio, 1/likelihood_ratio) if likelihood_ratio > 0 else 1.0
        
        return {
            'eve_detected': eve_detected,
            'likelihood_no_eve': likelihood_no_eve,
            'likelihood_with_eve': likelihood_with_eve,
            'likelihood_ratio': likelihood_ratio,
            'confidence': confidence,
            'observed_qber': observed_errors / n_bits if n_bits > 0 else 0,
            'expected_qber_no_eve': p_no_eve,
            'expected_qber_with_eve': p_with_eve
        }
    ...
    def run_random_basis_test(self, num_runs=1000, include_eve=True):
        ...
        for run in range(num_runs):
            qber, sifted_length, eve_was_active, observed_errors = self._simulate_qkd_protocol(include_eve)
            ...
            # Perform Eve detection using ML
            if sifted_length > 0:
                detection_result = self.eve_detection_ml(observed_errors, sifted_length)
                detection_result['actual_eve_present'] = eve_was_active
                detection_result['run_number'] = run
        ...
        self._analyze_detection_performance(detection_results)
    ...
    def _analyze_detection_performance(self, detection_results):
        """Analyze the performance of Eve detection"""
        if not detection_results:
            return
        
        # Classification metrics
        true_positives = sum(1 for r in detection_results 
                           if r['actual_eve_present'] and r['eve_detected'])
        true_negatives = sum(1 for r in detection_results 
                           if not r['actual_eve_present'] and not r['eve_detected'])
        false_positives = sum(1 for r in detection_results 
                            if not r['actual_eve_present'] and r['eve_detected'])
        false_negatives = sum(1 for r in detection_results 
                            if r['actual_eve_present'] and not r['eve_detected'])
        
        total = len(detection_results)
        accuracy = (true_positives + true_negatives) / total
        
        print(f"\n{'='*40}")
        print("EVE DETECTION ACCURACY")
        print(f"{'='*40}")
        print(f"Total simulations: {total}")
        print(f"Correct detections: {true_positives + true_negatives}")
    ...
    def _simulate_qkd_protocol(self, include_eve=True):
        ...
        if len(sifted_alice_bits) == 0:
            return 0.0, 0, eve_is_active, 0  # No sifted bits
        ...
        return qber, len(sifted_alice_bits), eve_is_active, errors
...
def main():
    ...
    test_configs = [
        ...
        {"noise_mean": 0.4, "noise_std": 0.2, "name": "High Noise"}
    ]
    ...
    for config in test_configs:
        ...
        # Run comparison test
        results_no_eve, results_with_eve = test.run_comparison_test(num_runs=100)
